...
angle_truewind=pi/2;
pcenter=[0;10];
x0=[-25;-25;pi/6;1;0];angle_rudder0=0;angle_sail0=-pi/3;
...
for time=0:timestep:timefinal
    ...
    if distancetotarget <= diainner/2
       ...
       %%%%%%%%
       if (distancetotarget < 1/sin(angle_ruddermax) || cos(pi+angle_target-angle_truewind) < 0) && (x0(4) > 0)
          %angle_actual=angle_truewind+pi;
       end
       %%%%%%%%
       if cos(x0(3)-angle_actual) > 0
          angle_rudder0=angle_ruddermax*sin(x0(3)-angle_actual);
